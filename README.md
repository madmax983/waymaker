...
| `timer-capability` | design document §11's timer semantics gain or lose a public function the pinned list does not have — a `TimerSpec::best_effort`, a `Timer::arm_or_downgrade`, a `PersistentClock::now_or_zero` — a pinned vocabulary declares a member the pin does not have, or the persistent-clock module names the boot clock, so a deadline that needs an RTC could be quietly served by a clock that restarts on every reset |

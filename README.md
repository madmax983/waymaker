...
| `storage-shapes` | an issue #130 operation shape stops being named by all four of `CLAUDE.md`, ADR 0046, `xtask::docs::STORAGE_SHAPES` and `waymaker-conformance`'s own shape table, the two tables disagree about who issues one, or the shape table is not where the gate looks for it |

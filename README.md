...
| `dispatch-wiring` | issue #36's dispatcher trait or its table gains a public function the pinned list does not have — a `Table::by_name`, a `Table::register` — either type gains a method at any visibility or a public field, or one of the two bodies that select a row names a row's label, so an activity could be reached by its name rather than by its number and issue #36's string-addressed-registry non-goal would be a convention rather than a build failure |

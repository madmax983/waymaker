...
| `failure-matrix` | a row of design document §14's failure-semantics table stops being named by all five of `CLAUDE.md`, ADR 0027, `xtask::docs::FAILURE_ROWS`, the rig's `Row::id` and a `#[test]` of its own name in `waymaker-drive`'s matrix, a row the table calls swept has no `#[test]` of its rig name in `waymaker-rig`'s matrix, or one of those files is not where the gate looks for it |

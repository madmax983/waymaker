...
| `ctx-facade` | issue #35's `Ctx` or the durable half it asks gains a public function the pinned list does not have — a `Ctx::record` that appends for itself, a fifth journal method — declares a fifth future, names a piece of on-media authority (`StableStorage`, `Reserved`, `RecordRef`, `Recovery`, `ReplayMachine`, `BankLayout`, `Swap`), or holds a `static`; or a synchronous-driver module names `waymaker_embassy`, so removing the façade would stop the protocol compiling |

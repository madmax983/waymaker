...
| `version-gate` | Design document §08's workflow versioning stops being the one that was reviewed: `RecordRef::VersionMarker`'s field set moves in either direction, the versioning vocabulary's surface moves, `VersionRange` grows a public field or a method at any visibility the pin does not list, or one of the three files that decide a version invokes `file!`, `line!`, `column!` or `module_path!` — §08 says a source-location hash is not stable identity, so a reformatting must not be a divergence |

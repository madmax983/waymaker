...
| `emulation-boot` | the emulated image stops being firmware, writes hand-written `unsafe` outside the two functions and one linker-symbol block ADR 0045 names, loses the reasoned `allow` its one exception rests on, disagrees with the harness about the prefix it prints, declares a binary that is not behind its feature, or names a core the toolchain does not pin and no stage runs |
...
| `recovery-surface` | the storage-backed recovery reader's public surface differs from the pinned list, so a `seek`, a `resume_at`, or a second route to an append offset could arrive without a reviewer writing it down; or `Recovery` is `Clone` — by a derive, a `cfg_attr`, or a handwritten `impl` — so one scan could hand out two writers at one offset |
...
| `storage-shapes` | an issue #130 operation shape stops being named by all four of `CLAUDE.md`, ADR 0047, `xtask::docs::STORAGE_SHAPES` and `waymaker-conformance`'s own shape table, the two tables disagree about who issues one, or the shape table is not where the gate looks for it |

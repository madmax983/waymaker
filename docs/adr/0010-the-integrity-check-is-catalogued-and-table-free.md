...
All three variants are in the repository — `crc32c`, `crc32c_nibble` and `crc32c_byte` in
`crates/waymaker-flash/tests/integrity.rs`, added for issue
[#17](https://github.com/madmax983/waymaker/issues/17), where
`every_candidate_computes_the_same_crc32c` holds all three to one answer. So the loops this
table was measured over are code somebody can rebuild rather than a paragraph.

CI still does not re-run the *measurement* — there is no cycles budget for it to gate, which
is [the point](#what-would-revisit-this) — so the numbers above are a dated measurement
rather than a checked one, and they are stated precisely enough to be checked by hand.

...
## Verification (codex-watch run, 2026-09-13, reviews 5188824416 + 5188852872, review_comments 3998151689 + 3998178352)

- Reviews 5188824416, 5188852872: header cards only (621 bytes each, Codex
  review header/footer + reviewed SHA, no findings) — declined as empty.
- Review comment 3998151689 (P2, "Classify procedural attributes embedded in
  macro bodies"): verified real. The macro-body scanner classified nested
  macro *invocations* but not attribute-shaped tokens, so
  `macro_rules! wrapper { () => { #[evil::startup] fn x() {} } }` smuggled an
  external procedural attribute past every scan while `wrapper!()` was trusted
  as local. Fix in `xtask/src/emulate.rs`: `macro_body_attributes` pairs
  `#`/`#!` with the following bracketed group inside opaque macro bodies and
  parses each attribute; `macro_body_attributes_are_accounted_for` classifies
  each with the same `AttributeGate` the top-level walk applies (builtins
  ignored, std derives and `cfg_attr` accounted for, anything else — including
  unparseable shapes — fail-closed); `nested_invocation_paths` skips `#!` so
  inner attributes are not misread as invocations. Same-class sibling fixed in
  the same pass: `visit_item_macro` returned early for `macro_rules!` without
  visiting `node.attrs`, so `#[evil::startup] macro_rules! wrapper {...}`
  skipped every scan — each definition attribute now gets the same gate
  treatment. 8 new tests (6 body-attribute + 2 definition-attribute, refusal /
  keep-green pairs).
- Review comment 3998178352 (P2, "Restore page-sized reads for erased witness
  tails"): verified real — the pushed branch (27851682fc) was squashed from a
  pre-#126 tree, so its `Witness::read` reverted #126's page-sized tail scan
  to per-slot reads. `crates/waymaker-rig/src/witness.rs` and
  `crates/waymaker-rig/tests/witness.rs` restored to current main (e813d1c),
  retaining `verify_erased_to_end` and its regression tests. NOTE: the same
  stale-squash root cause leaves ~28 other files at their 1420485 versions
  (conformance, spec, rig tests, size-probe, xtask profile) — the branch did
  not change them, so merges stay safe, but the PR diff shows them as
  deletions; intentionally left alone per minimal scope (only the Codex-flagged
  witness files were restored).

GATES (2026-09-13):
- `cargo fmt --all --check`: clean
- `cargo clippy --locked --workspace --all-targets -- -D warnings`: clean
- `cargo test --locked -p xtask --lib`: 1140 passed, 0 failed (incl. the 8 new
  macro-body/definition attribute tests)
- `cargo test --locked -p waymaker-rig`: all suites green (incl. witness tail
  tests)
- `cargo xtask check-layering`: ok (56 rules)
NOTE: the full `cargo test -p xtask` (incl. `workspace_policy` + `size_budgets`
integration tests) was not run to completion — the `workspace_policy`
integration test hung in this sandbox (an unknown second `cargo test -p xtask`
process was also running in the worktree and both were killed); the lib suite
+ layering gate cover the changed code.

## Codex P2s, reviews 5188824416 / 5188852872 (codex-watch run, 2026-09-12)

Reviews 5188824416 (`359f9b231b`) and 5188852872 (`27851682fc`): header cards
only, no actionable findings — declined with reason. Each carries one real
review comment, handled below.

### P2 — classify attributes embedded in macro bodies (review_comment 3998151689) — FIXED

**File**: `xtask/src/emulate.rs`

**Verified real** via RED-first tests: with the gate call removed, the three
refusal tests fail with 0 violations — `macro_rules! wrapper { () => {
#[evil::startup] fn x() {} } }` followed by `wrapper!()` smuggled the
external procedural attribute past every scan, exactly the fail-open Codex
describes. The definition scan looked for `include!` / `mod` / invocation
shapes in the opaque body, but an attribute is none of those, and the
attribute walk only visits parsed attributes, which body tokens never are.

**Fix**: `macro_body_attributes` pairs `#` (either spacing — `#!` lexes `#`
joint) or `#!` with the bracketed group that follows, descends into groups,
and parses each shape with `syn::Attribute::parse_outer` / `parse_inner`;
`macro_body_attributes_are_accounted_for` classifies each via the same
`AttributeGate` the top-level walk applies (builtins ignored, std derives
and `cfg_attr` accounted for, anything else fail-closed — including shapes
the scanner cannot parse). The gate is wired into `visit_item_macro`'s
`macro_rules` branch.

Two adjacent holes closed in the same pass:
- The invocation-shape walk read an inner attribute's `#!` as a pathless
  macro invocation (`!` + bracket group with an empty path → `None` →
  fail-closed): a false positive on every builtin inner attribute a body
  carries. A `!` immediately preceded by `#` is now skipped there — sound
  because `#` is not a path token, so no invocation path can hide behind
  it, and the attribute scan classifies (or fail-closes) the shape instead.
- Attributes on the `macro_rules!` definition itself were never visited:
  the branch returned early without `record(&node.attrs, ..)`, so
  `#[evil::startup] macro_rules! wrapper { ... }` escaped classification
  the same way. Each definition attribute now goes through
  `record_attribute` before the early return.

**Tests** (8 new; RED first — the 4 refusal tests fail with 0 violations
without the fixes, the 4 keep-green tests pass before and after):
`an_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption`,
`a_builtin_attribute_nested_in_a_macro_rules_body_keeps_the_exemption`,
`a_std_derive_nested_in_a_macro_rules_body_keeps_the_exemption`,
`a_cfg_attr_emitting_an_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption`,
`an_inner_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption`,
`an_inner_builtin_attribute_nested_in_a_macro_rules_body_keeps_the_exemption`,
`an_external_attribute_on_a_macro_rules_definition_refuses_the_exemption`,
`a_builtin_attribute_on_a_macro_rules_definition_keeps_the_exemption`.

### P2 — restore batch reads for erased witness tails (review_comment 3998178352) — FIXED

**Files**: `crates/waymaker-rig/src/witness.rs`, `crates/waymaker-rig/tests/witness.rs`

**Verified real**: the branch's rebase onto the pre-#126 base left
`Witness::read` walking every erased slot with a separate storage read and
decode, reverting main's #126 page-sized tail scan (bolt-measured journal
Ir -16.05%).

**Fix**: restores `verify_erased_to_end` — after the first erased slot, the
tail is verified a page at a time instead of a slot at a time — plus its two
regression tests, byte-identical to #126
(`a_scan_with_a_wide_erased_tail_still_reports_the_marks_before_it`,
`a_single_programmed_byte_anywhere_in_a_wide_erased_tail_is_a_hole`).

## Verification (codex-watch run, 2026-09-12, reviews 5188824416 / 5188852872)

- `cargo fmt --all --check`: clean
- `cargo clippy --locked --workspace --all-targets -- -D warnings`: clean
- `cargo test --locked --offline -p xtask --lib`: all green (incl. the 8 new attribute tests)
- `cargo test --locked --offline -p waymaker-rig`: all green (incl. the 2 restored witness tests)
- `cargo xtask check-layering`: ok

## New Codex findings (codex-watch run, 2026-09-12, review 5189373769) — handled

### Automated-review header card (review 5189373769) — nothing actionable

The review body is Codex's automated-review header card ("Here are some
automated review suggestions for this pull request", reviewed commit
`d0021ff3ba`); it contains no findings or suggestions, so there was
nothing to fix.

### P2 — restore per-operation watchdog enumeration wording (review_comment 3998577500) — DECLINED

Anchors to `docs/adr/0023-…md` line 55 ("a reset before the sequence").
Verified: the sentence Codex quotes no longer exists — main's #145 (issue
#88, commit 9bec8a5, "Fix stale watchdog enumeration statement in ADR
0023") already corrected it through the proper ADR-amendment process, and
this branch is rebased on that main. The implementation agrees:
`crates/waymaker-fault/tests/enumeration.rs` enumerates `Progress::None`
with `Interruption::Watchdog` before *every* operation ("Watchdog: one
before *each* op" in `the_count_is_the_arithmetic_the_sequence_implies`;
`(0, None, Watchdog)` and `(1, None, Watchdog)` both present in
`an_operation_that_mutates_nothing_contributes_no_duplicate_worlds`). ADR
0023 is accepted frozen history; editing it on a review branch was never
an option, and there is nothing left to change.

### P2 — propagate crate-root macro imports to child scans (review_comment 3998577504) — FIXED

**File**: `xtask/src/emulate.rs`

**Verified real**: `include_declarations` initialized
`has_macro_use_extern` from `has_macro_use_extern_crate(file)` — the
*child* file's own syntax tree. A `#[macro_use] extern crate evil;` in the
crate root puts evil's macros in textual scope for every module, so a bare
`assert!(...)` in `arm.rs` may invoke evil's macro while the walk of
`arm.rs` saw no `#[macro_use]` and classified it as the builtin — an
invisible expansion could declare/include the gated file under the
separately gated `mod xtensa;` exemption. RED first: with only the
`#[macro_use]` attribute whitelisted, the new test
`a_macro_use_extern_crate_in_the_root_shadows_builtins_in_child_files`
failed with 0 violations before the fix (exactly the miss), and passes
with 1 after.

**Fix**: `xtensa_gated_module` computes `macro_use_extern_in_root` once
from the crate root's parse tree and `include_declarations` ORs it into
every file's `has_macro_use_extern` — root-propagated rather than
crate-wide, the precise shape of the hole; the error direction stays
fail-closed. Companion change: `"macro_use"` added to
`BUILTIN_ATTRIBUTES` — the attribute itself is inert (names no expansion),
and the `AttributeGate` shadowing guards (`use`-imports, local imports,
external globs are checked before the whitelist) already fail closed on a
shadowed `#[macro_use]`; without it, any root carrying the triggering
`#[macro_use] extern crate` would unconditionally refuse the exemption.

## Verification (codex-watch run, 2026-09-12, review 5189373769)

- `cargo fmt --check`: clean
- `cargo clippy --all-targets -- -D warnings`: clean
- `cargo test -p xtask --lib`: all green (1141 passed, incl. the new test)
- `cargo xtask check-layering`: ok (56 rules)

## New Codex findings (codex-watch run, 2026-09-13, review 5189784620) — all handled

### Automated-review header card (review 5189784620) — nothing actionable

The review body is Codex's automated-review header for commit `0ec4ec176d`;
it contains no findings or suggestions, so there was nothing to fix.

### P2 — use address arithmetic for the BSS linker range (review_comment 3998898841) — FIXED

**File**: `crates/waymaker-emu/src/xtensa.rs` (`zero_bss`)

**Verified real**: `static mut _sbss: u32;` declares a lone `u32`, so a pointer
derived from it via `addr_of_mut!` carries provenance for that single object
only. The old loop advanced a `*mut u32` cursor with `ptr::add(1)` — producing
one-past-the-end on the first advance (dereferencing it on the second
iteration) and going further out of bounds after that. Under strict provenance
that is undefined behavior from the second word on, even though the linker
lays `.bss` out contiguously after `_sbss`. A real UB-level correctness issue
in hand-written `unsafe` startup code, exactly the class Mark wants fixed.

**Fix**: per the comment's own suggestion — the loop now runs over integer
addresses: both linker bounds are reduced to `usize` (exposing the addresses)
and each iteration rebuilds the write pointer with `addr as *mut u32` before
the volatile word store, so no pointer ever inherits `_sbss`'s provenance.
Both ends are word-aligned by `memory-xtensa.x` (the earlier
review_comment 3995859254 fix), so every word write stays inside the section;
an empty `.bss` (`_sbss == _ebss`) writes nothing.

**Tests** (RED first — the new test failed against the `ptr::add` loop with
"zero_bss must reduce the linker bounds to integer addresses before looping",
passes after): `the_xtensa_bss_clear_uses_address_arithmetic` in
`xtask/src/emulate.rs` asserts `zero_bss` reduces the bounds with `as usize`,
rebuilds the pointer per word with `as *mut u32`, still clears with
`write_volatile`, and contains no `.add(`. The Xtensa guest still compiles and
links for `xtensa-esp32s3-none-elf` with the `esp` toolchain (release image
built with the harness's link args).

## Verification (codex-watch run, 2026-09-13, review 5189784620)

- `cargo fmt --all --check`: clean
- `cargo clippy --locked --workspace --all-targets --no-default-features -- -D warnings`: clean
- `cargo test --locked -p xtask`: all green (1147 lib tests incl. the new one; integration suites green)
- `cargo xtask check-layering`: ok
- Xtensa guest: compiles and links with the `esp` toolchain (`xtensa-esp32s3-none-elf`, `-Z build-std=core`); guest boot remains CI's job (Espressif QEMU fork unavailable in the sandbox)
- Note: one `workspace_policy` integration test (`a_change_that_only_works_on_the_host_fails_the_firmware_build`) failed mid-run on "No space left on device" — the 512 MiB tmpfs `/tmp` cannot hold the scratch-workspace build; re-ran green with `TMPDIR` on the main disk. Environmental, unrelated to the change.

## New Codex findings (codex-watch run, 2026-09-13, review 5190104911) — all handled

### Automated-review header card (review 5190104911) — nothing actionable

The review body is Codex's automated-review header; it contains no findings beyond the two inline review comments below.

### P2 — conflicting macro imports across scopes fail closed (review_comment 3999109484) — FIXED

**File**: `xtask/src/emulate.rs`

**Verified real**: the import walk is file-wide but separate lexical scopes may
import the same local name from different crates — which compiles fine (only
same-scope duplicates are E0252). The old `record_name` did a plain
`HashMap::insert`, so the last scope's mapping silently won: with `mod
unvetted { use evil::hprintln; ... }` before `mod vetted { use
cortex_m_semihosting::hprintln; ... }`, the pinned
`(cortex_m_semihosting, hprintln)` exemption was granted to invocations that
actually resolve to evil's macro — fail-open, contradicting the documented
fail-closed invariant. RED first: the new conflict test produced 0 violations
before the fix, exactly the miss.

**Fix**: `record_macro_import_name` (extracted from `external_macro_imports` to
module scope — the extraction also keeps the parent under clippy's
`too_many_lines` limit) now fails closed on disagreement: a second mapping for
a local name that differs in crate or original name — or a name already bound
through an unresolvable local path — drops the recorded external mapping and
records the name in `local_imports` instead, so classification refuses the
exemption. An identical mapping recorded twice is not a conflict and keeps its
entry.

**Tests** (RED first — the conflict test failed with 0 violations before the
fix; the agreement guard passed before and after):
`conflicting_macro_imports_across_scopes_fail_closed` (two scopes, evil vs the
pinned pair → 1 violation on `xtensa.rs`) and
`agreeing_macro_imports_across_scopes_keep_their_mapping` (same `(evil,
hprintln)` mapping twice → still exactly 1 violation, no behavior change).

### P2 — detect `macro_use` emitted by `cfg_attr` (review_comment 3999109488) — FIXED

**File**: `xtask/src/emulate.rs`

**Verified real**: `has_macro_use_extern_crate` only matched a directly written
`#[macro_use]`, so `#[cfg_attr(target_arch = "arm", macro_use)] extern crate
evil;` in the crate root left the root propagation false and a child file's
`assert!()` was accepted as the compiler builtin — even though on ARM it
resolves to evil's macro, whose invisible expansion the checker cannot read.
RED first: the new test produced 0 violations before the fix, exactly the
miss.

**Fix**: the finder now also recognizes `cfg_attr` attributes that can emit
`macro_use` — parsing the argument list the same way `cfg_attr_applied_paths`
does (skip the predicate, inspect the emitted metas, recurse into nested
`cfg_attr`), without evaluating the predicate, so any configuration that could
emit it counts. The error direction is fail-closed: a `cfg_attr` emitting
anything else does not trip the wholesale-import handling.

**Tests** (RED first — both positive tests failed with 0 violations before the
fix; the negative guard passed before and after):
`a_cfg_attr_macro_use_extern_crate_in_the_root_shadows_builtins_in_child_files`
(direct form → 1 violation on `xtensa.rs`),
`a_nested_cfg_attr_macro_use_extern_crate_counts_as_macro_use` (nested form →
1 violation), and
`a_cfg_attr_extern_crate_without_macro_use_keeps_the_gate_green`
(`cfg_attr(.., allow(dead_code))` → gate stays green).

## Verification (codex-watch run, 2026-09-13, review 5190104911)

- `cargo fmt --all --check`: clean
- `cargo clippy --locked --workspace --all-targets --no-default-features -- -D warnings`: clean
- `cargo test --locked -p xtask --no-default-features --lib`: all green (1155 lib tests incl. the 5 new ones)
- `cargo xtask check-layering`: ok (56 rules)

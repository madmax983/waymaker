...
- **`waymaker_flash::append::Journal`, and the two types beside it.** The façade's own
  ...
  *are* banned, so this is a hole in a list presented as exhaustive rather than an open
  door.
- **That the manifest entry can be dropped.** `waymaker-drive`'s dependency on
  `waymaker-embassy` is not optional, so the `drive-facadeless` build still resolves and
...
modules and the `drive-facadeless` stage builds that configuration for the part, which is the
claim as a compile; `ctx-facade` is the scan beside it, and it reads every module of the
crate rather than a list, so a module added tomorrow is covered.

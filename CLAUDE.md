...
world is not asked. Codex found, on review of this change's first version, that a caller
could still skip the check: nothing stopped `activities.perform(intent, wrong_bytes, out)`
called directly, since `Activities::perform` still took a bare `&[u8]`.
`Activities::perform` now takes a `CheckedInput` instead — a type with a private field that
only `Dispatchable::perform` can build — so there is no bytes a direct call could pass it
that the check did not vouch for either. `Effect::redelivering` threads the same binding
through with no kernel-boundary change: `decide` already checks the workflow's current
request against history with `ReplayMachine::intent` before the redelivery row is reached,
so the request `redelivering` binds is the one the kernel already vouched for — issue #92's
own guess that this would need `Resolve::Redeliver` widened did not hold up once the call
site was read. `effect-protocol` pins all three new methods on `EFFECT_PROTOCOL_SURFACE` and
`EFFECT_TYPE_METHODS`; none is one of §07's storage steps, so `EFFECT_STEP_BODIES` is
unchanged. The gated `layers` figure moves to **12732 B** of 13312, 80 B below ADR 0036's
12820 B and unmoved by `CheckedInput`, which the optimiser erases — the wider `DurableIntent`
and the new checked call cost less than the free `kind` argument they replace, on this
optimiser, at this setting; no raise is asked for. See
[ADR 0045](docs/adr/0045-a-durable-intent-carries-its-request-and-perform-checks-it.md).

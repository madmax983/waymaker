...
- **The corpus** — [`crates/waymaker-flash/tests/corpus/v1`](crates/waymaker-flash/tests/corpus/v1/README.md),
  fifteen files of frozen bytes, run by `crates/waymaker-flash/tests/corpus.rs` as the
  `corpus` CI stage. It is the only thing here that can notice a *renumbering*: the encoder
  takes a kind's number from `RecordRef::kind` and the decoder matches the same constants, so
  swapping two leaves every round trip, every property test and every crash sweep green. The
...
- **The `wire-format` rule** — the frozen constants, the record numbering in both directions,
...
step 5 of the swap is the format transition. ADR 0037 states the fleet end to end and the
order the two images ship in.
...
- **That a corpus file is the byte sequence a v1 device really wrote.** The corpus was
  produced by an encoder written from the field list rather than from `frame.rs`, and it
  reproduces `tests/frame.rs`'s golden frames byte for byte — which is a cross-check between
  two independent implementations and not a reading off a device. No board has written a
...
was watched failing on six mutations before it was closed, the sharpest being a renumbered
`RUN_STARTED` and a kind added to the kernel that nothing wrote down. Migration is §10's swap

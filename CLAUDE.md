...
- **A constant added from a sibling module, for the three bans issue #99 added.** Issue
  ...
  a silent pass — but it is not a read of the real value either. And all three bans read only
  *inherent* `impl` blocks, because `inherent_impl_bodies` skips a header naming `for`: its
  methods belong to the trait, not the type. So a constant declared inside a hand-written
  `impl SomeTrait for Effect` is invisible to them. `EFFECT_NO_SELF_LITERAL` already refuses
  `DurableIntent` and `Dispatchable` a trait `impl` at all; `Effect` itself, every
  `source::BOUNDARY_TYPES` type, and `ClockKind` have no such backstop.

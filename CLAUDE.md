...
- **A constant added from a sibling module, for the three bans issue #99 added.** Issue
  ...
  a silent pass — but it is not a read of the real value either. Four more routes Codex found
  ...
  rather than resolved by whichever value a map happens to keep last. And a leading attribute
  on the same line as a declaration — `#[rustfmt::skip] pub const X: Self = ..;`, which
  survives `cargo fmt` — is stripped before the line is read, `next_impl_line`'s reason, so
  it no longer hides the declaration entirely.

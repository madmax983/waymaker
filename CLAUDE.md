...
- **A §07 step reachable only on one branch, or written by a macro.** `effect-protocol`
  ...
  counting parentheses and brackets as well as braces, in a body that declares no closure —
  and every proof of durable intent to be built after the commit barrier. Codex round 2 is
  why the depth counts more than braces: `false.then(|| ...)` has none. What a scanner still
  cannot say is whether the *path* that reaches the construction is the path that took the
  steps — a `?` on a call above them short-circuits without a `return` — nor what a macro
  expands to, since a macro-generated `fn` is in no `impl` body it reads. `capacity-reserve`
  records the first limit for the same reason, and `crates/waymaker-drive/tests/crash.rs` is

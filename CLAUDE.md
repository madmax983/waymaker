...
owed. Issue #24's commit seal and two-barrier writer then took it to 14668 B against the same
16 KiB gate, and
[ADR 0019](docs/adr/0019-the-commit-seal-is-a-masked-repeat-and-the-writer-is-a-typestate.md)
splits that: 2420 B is the seal reaching the codec and the two readers, and 1272 B is the
writer with the probe section that keeps it alive. The gate is **not** raised again, and
1716 B of headroom is what the rest of rung 0.2 has to fit in. A third of the measured
...
torn part-way through its own program always ends in erased bytes, and a seal is bound to the
frame it covers. Those three together make "sealed but incomplete" a state that cannot be

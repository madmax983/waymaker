...
- **A dispatch-wiring function added from a sibling module, and what a name really reaches.**
  ...
  `ctx.rs` adds a `by_name` with the rule silent. And it compares *names*, so that a row's
...
- **That the `static` ban still refuses everything it was written for.**
  `ctx-facade`'s hidden-state scan sets the `'static` *lifetime* aside before it looks for
  the identifier, because `&'static str` is what compile-time metadata is spelled as and the
  rule is about a `static` item. An item is `static NAME:` and never `'static`, so the
  narrowing loses nothing — and `a_static_item_beside_a_static_lifetime_is_still_reported`
...
bank, and the façade hands the world `out[..result_bytes]`. So the validation is not a check
...
async `Ctx` is here — issue #35, below — and the dispatcher and in-boot sleep are the rest
of 0.4. The

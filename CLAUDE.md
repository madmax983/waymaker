...
- **Which column a generic layer's bytes land in.** ADR 0029 already records that fat LTO
  ...
  is *entirely* generic, so almost all of it lands in `probe`: of postcard's 156 B the
  symbol table attributes 158 B to the probe and −2 B to the layers. The whole-image
...
still reads its eight-byte handle with `try_into`. And enabling postcard costs **156 B** of
code flash against the `facade` row, with the bridge alone costing 32 B. The *gated* row —
...
tabulated, and `check_probe_mirrors` makes it compulsory — a layer feature with no mirror
fails the `size-probe` rule rather than producing a quiet row of zero. Where the 156 B lands
is worth reading twice, and is written down rather than smoothed over: the symbol table
charges 158 B of it to the probe and −2 B to the layers, because this codec is the first
...
Three CI stages are new, and they exist because every other stage passes
`--no-default-features`: `codec-lint`, `codec-test` and `codec-firmware` are what lint this
code, test it, and build it for the part. `cargo xtask coverage` runs

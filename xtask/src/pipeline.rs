...
pub const STAGES: &[Stage] = &[
    ...
    Stage {
        name: "emu-lint",
        job: "firmware",
        // `probe-lint`'s argument, met a second time. The emulated image's binary is behind
        // `required-features`, so the `lint` stage — `--all-targets --no-default-features`
        // on the host — never compiles it, and `cargo xtask emulate` builds it rather than
        // linting it. Without this stage the one crate in the workspace that carries
    ...
    Stage {
        name: "emulate",
        job: "emulation",
        // Its own job because it needs an emulator no rustup profile carries, and because of
        ...
        // to move one. ADR 0039 is where that is argued rather than left to a reader.
        //
        // The command takes no arguments so that this table can compare it against the
        // workflow byte for byte, which is `size`'s and `profile`'s reason.
        command: "cargo --locked xtask emulate",
        in_hook: false,
        why: "a crate that compiles for the part is not a crate that runs on one: `cargo build --lib` never links",
    },
    Stage {
        name: "layering",
        job: "layering",

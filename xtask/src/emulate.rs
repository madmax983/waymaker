...
/// rather than a panic.
/// Removes the previous run's serial log before the new guest starts.
///
/// A stale log is removed; a missing one is fine. Anything else is refused by
/// the caller: the old behavior ignored the removal error, which let the first
/// polling iteration parse the old census before QEMU had opened and truncated
/// the file — the harness would then kill the new guest and report a
/// successful run that never produced the observed output.
fn remove_stale_log(log: &Path) -> Result<(), String> {
    // Codex P2 (review_comment 3998828462): ignoring the removal error lets
    // the first polling iteration parse the old census before QEMU has opened
    // and truncated the file, so the harness kills the new guest and reports
    // a successful run that never produced the observed output. Only a
    // missing log is fine.
    match std::fs::remove_file(log) {
        Ok(()) => Ok(()),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(format!(
            "cannot remove stale log {}: {error}",
            log.display()
        )),
    }
}
...
fn xtensa_gated_module(root_path: &str, files: &[&crate::size::LayerSource], path: &str) -> bool {
    ...
    // Whether the crate carries `#[macro_use] extern crate`: its macros
    // land in textual scope for every module in the crate, so a bare
    // invocation in a child file may name the dependency's macro rather than
    // the builtin — even though the child file itself carries no `#[macro_use]`
    // and the per-file flag the walk initializes cannot see that (Codex P2,
    // `review_comment` 3998577504). The root's flag is propagated into every
    // file's walk; the error direction is fail-closed. A `macro_rules!` that
    // expands to `#[macro_use] extern crate` counts the same way — the
    // generated declaration is invisible to the item scan, and its
    // invocation site is unknown, so the shape is detected in every file
    // and the flag propagates crate-wide (Codex P2, `review_comment`
    // 3999217503).
    let macro_use_extern_in_root = files.iter().any(|source| {
        let Ok(tree) = crate::parse::parse_rust(&source.contents) else {
            // An unparsable file fails closed in the loop below; here it
            // contributes nothing, and the loop's own fail-closed stands.
            return false;
        };
        if macro_body_emits_macro_use_extern_crate(&tree) {
            return true;
        }
        if source.path != root_path {
            return false;
        }
        has_macro_use_extern_crate(&tree)
    });
    for source in files {
        let Ok(tree) = crate::parse::parse_rust(&source.contents) else {
        ...
        let Some(inclusions) = include_declarations(
            &tree,
            &dirs.file_dir,
            &local_macros,
            macro_use_extern_in_root,
        ) else {
...
///
/// `macro_use` is whitelisted here for the attribute itself: it is inert and
/// names no expansion. The macros a `#[macro_use] extern crate` puts in
/// textual scope are a separate question, answered on the macro-invocation
/// side ([`Inclusions::has_macro_use_extern`]).
...
///   is never trusted, and an import through a shadowed root is recorded as
///   unresolvable, never as the pinned pair (Codex P2, `review_comment`
///   3999003978);
...
/// Whether a `use` root resolves inside the crate or to the builtin namespace.
fn is_macro_import_local_root(root: &str) -> bool {
    matches!(root, "crate" | "self" | "super" | "core" | "std")
}

/// Whether a `use` root is a local path the checker cannot resolve through
/// the module tree.
fn is_unresolvable_macro_import_root(root: &str) -> bool {
    matches!(root, "crate" | "self" | "super")
}

/// Whether a `use` root names the builtin namespace.
fn is_builtin_macro_import_root(root: &str) -> bool {
...
///
/// A name the checker cannot resolve to one binding fails closed: it is
/// dropped from [`MacroImports::imports`] and recorded in
/// [`MacroImports::local_imports`] instead.
fn record_macro_import_name(
    out: &mut MacroImports,
    shadowed_roots: &HashSet<String>,
    root: &str,
    local: String,
    original: String,
) {
    /// Move `local` to the unresolvable set, dropping any recorded
    /// external mapping for it.
    fn unresolvable(out: &mut MacroImports, local: String) {
        out.imports.remove(&local);
        out.local_imports.insert(local);
    }

    if shadowed_roots.contains(root) {
        // A local item shadows the extern prelude, so the textual root
        // may name the local definition instead of the extern crate: the
        // import is not provably the pinned one and fails closed.
        unresolvable(out, local);
    } else if is_unresolvable_macro_import_root(root) {
        unresolvable(out, local);
    } else if !is_builtin_macro_import_root(root) {
        let conflict = out.local_imports.contains(&local)
            || out
                .imports
                .get(&local)
                .is_some_and(|(mapped_root, mapped_original)| {
                    mapped_root != root || mapped_original != &original
                });
        if conflict {
            // Separate lexical scopes import the same local name from
            // different crates (or under different originals): the
            // file-wide walk cannot tell which binding an invocation
            // resolves to, so the last `insert` must not silently win —
            // granting the pinned exemption on the wrong mapping is
            // fail-open. Disagreement fails closed (Codex P2,
            // `review_comment` 3999109484).
            unresolvable(out, local);
        } else {
...
///
/// The root is trusted to name the extern crate only when no local item
/// shadows it: `mod cortex_m_rt { pub use evil::entry; }` plus `use
/// cortex_m_rt::entry;` makes the bare name apply evil's macro, so an import
/// through a shadowed root is recorded in [`local_imports`] and fails closed
/// instead of granting the pinned exemption on the textual `(crate, name)`
/// pair (Codex P2, `review_comment` 3999003978).
fn external_macro_imports(file: &syn::File, shadowed_roots: &HashSet<String>) -> MacroImports {
    fn walk(
        tree: &syn::UseTree,
        root: Option<&String>,
        shadowed_roots: &HashSet<String>,
        ...
        match tree {
            syn::UseTree::Path(path) => {
                ...
                walk(&path.tree, next.as_ref(), shadowed_roots, out);
            }
            syn::UseTree::Name(name) => {
                if let Some(root) = root {
                    record_macro_import_name(
                        out,
                        shadowed_roots,
                        root,
            ...
            syn::UseTree::Glob(_) => {
                if let Some(root) = root {
                    if !is_macro_import_local_root(root) {
            ...
            syn::UseTree::Group(group) => {
                for item in &group.items {
                    walk(item, root, shadowed_roots, out);
                }
            }
        }
    }
    ...
    impl<'ast> syn::visit::Visit<'ast> for Collector<'_> {
        fn visit_item_use(&mut self, node: &'ast syn::ItemUse) {
            walk(&node.tree, None, self.shadowed_roots, &mut self.out);
...
/// `#[cfg_attr(.., macro_use)]` emits the attribute when its predicate holds,
/// and the checker does not evaluate predicates, so a `cfg_attr` that can
/// emit `macro_use` counts the same way (Codex P2, `review_comment`
/// 3999109488).
fn has_macro_use_extern_crate(file: &syn::File) -> bool {
    /// Whether a `cfg_attr` argument list can emit `#[macro_use]`: the
    /// emitted attributes are the metas after the predicate, recursing into
    /// nested `cfg_attr` lists the way [`cfg_attr_applied_paths`] does. The
    /// predicate is not evaluated, so a list that can emit `macro_use` under
    /// any configuration counts — the error direction is fail-closed.
    fn cfg_attr_emits_macro_use(list: &syn::MetaList) -> bool {
        let Ok(metas) = list.parse_args_with(
            syn::punctuated::Punctuated::<syn::Meta, syn::Token![,]>::parse_terminated,
        ) else {
            return false;
        };
        let mut emitted = metas.iter();
        emitted.next(); // The cfg predicate: not an emitted attribute.
        emitted.any(|meta| match meta {
            syn::Meta::Path(path) => path.is_ident("macro_use"),
            syn::Meta::List(nested) if nested.path.is_ident("cfg_attr") => {
                cfg_attr_emits_macro_use(nested)
            }
            _ => false,
        })
    }

    /// Whether one attribute applies `macro_use` to the `extern crate`,
    /// directly or through `cfg_attr`.
    fn attr_applies_macro_use(attr: &syn::Attribute) -> bool {
        ...
        list.path.is_ident("cfg_attr") && cfg_attr_emits_macro_use(list)
    }
    ...
    impl<'ast> syn::visit::Visit<'ast> for Finder {
        fn visit_item_extern_crate(&mut self, node: &'ast syn::ItemExternCrate) {
            if node.attrs.iter().any(attr_applies_macro_use) {
                self.found = true;
            }
...
/// Whether any `macro_rules!` body in the file can generate
/// `#[macro_use] extern crate`.
///
/// A `macro_rules!` body is opaque tokens to `syn`: a macro expanding to
/// `#[macro_use] extern crate evil;` puts the dependency's macros in textual
/// scope at every invocation site, but [`has_macro_use_extern_crate`] cannot
/// see the generated declaration (Codex P2, `review_comment` 3999217503). The
/// invocation site is unknown, so the shape is detected here — scanning the
/// body token stream for the `#[macro_use] extern crate` token shape, with an
/// optional `pub` between the attribute and the item — and the flag
/// propagates crate-wide like the root flag in [`xtensa_gated_module`]: the
/// error direction is fail-closed. A body that merely names the tokens apart
/// — `macro_use` in a string literal, or an `extern crate` without the
/// attribute — does not match.
fn macro_body_emits_macro_use_extern_crate(file: &syn::File) -> bool {
    /// One flattened token: punctuation as its char, identifiers as their
    /// string, group delimiters as their bracket chars. Literals carry no
    /// part of the shape and are skipped; groups are flattened in place with
    /// their delimiters preserved, so `#[macro_use]` flattens to `#`, `[`,
    /// `macro_use`, `]`.
    #[derive(PartialEq, Eq)]
    enum Atom {
        Punct(char),
    ...
    fn flatten(stream: &proc_macro2::TokenStream, out: &mut Vec<Atom>) {
        for tree in stream.clone() {
            match tree {
                proc_macro2::TokenTree::Group(group) => {
                    ...
                    out.push(Atom::Punct(open));
                    flatten(&group.stream(), out);
                    out.push(Atom::Punct(close));
                }
                proc_macro2::TokenTree::Ident(ident) => {
                    out.push(Atom::Ident(ident.to_string()));
                }
                proc_macro2::TokenTree::Punct(punct) => {
                    out.push(Atom::Punct(punct.as_char()));
                }
    ...
    /// Whether the atom slice starting at `start` is the `#[macro_use]
    /// extern crate` shape, allowing an optional `pub` between the attribute
    /// and the item.
    fn shape_at(atoms: &[Atom], start: usize) -> bool {
        let is = |index: usize, want: &Atom| atoms.get(start + index) == Some(want);
        let hash = Atom::Punct('#');
        let open = Atom::Punct('[');
        let macro_use = Atom::Ident("macro_use".to_owned());
        let close = Atom::Punct(']');
        let public = Atom::Ident("pub".to_owned());
        let external = Atom::Ident("extern".to_owned());
        let krate = Atom::Ident("crate".to_owned());
        if !(is(0, &hash) && is(1, &open) && is(2, &macro_use) && is(3, &close)) {
            return false;
        }
        let item = if is(4, &public) { 5 } else { 4 };
        is(item, &external) && is(item + 1, &krate)
    }
    ...
    impl<'ast> syn::visit::Visit<'ast> for Finder {
        fn visit_item_macro(&mut self, node: &'ast syn::ItemMacro) {
            if node.mac.path.is_ident("macro_rules") {
                ...
                flatten(&node.mac.tokens, &mut atoms);
                if (0..atoms.len()).any(|start| shape_at(&atoms, start)) {
                    self.found = true;
                }
            }
...
struct Inclusions<'ast> {
    ...
    /// `#[macro_use] extern crate` in this file — or in the crate root, whose
    /// textual-scope macros reach every module ([`include_declarations`]
    /// propagates the root's flag into every file's walk): the macros may
...
impl<'ast> Inclusions<'ast> {
    ...
    /// `std::` names the builtin namespace directly — unless a local item
    /// shadows the root, which the shadow check above rejects first (Codex
    /// P2, `review_comment` 3998765648). A path rooted at
    ...
    fn macro_path_is_accounted_for(&self, path: &syn::Path) -> bool {
        ...
        if path.segments.len() > 1 {
            let root = path
                ...
                .map(|segment| segment.ident.to_string())
                .unwrap_or_default();
            // A local item shadows the extern prelude — `mod core { pub use
            // evil::assert; }` makes `core::assert!()` name the re-exported
            // foreign macro, not the builtin namespace — so the shadow check
            // runs before the builtin-root branch (Codex P2,
            // `review_comment` 3998765648).
            if self.shadowed_roots.contains(&root) {
                return false;
            }
        ...
        // Wholesale imports resolve before the crate-wide local-name set: a
        // `#[macro_use] extern crate` at the root (or an external glob) puts
        // the dependency's macros in textual scope for every module, so a
        // bare invocation may name the dependency's macro even when an
        // unrelated module defines a same-named `macro_rules!` — the
        // over-approximated set does not put that definition in textual
        // scope at the invocation site (Codex P2, `review_comment`
        // 3998765650).
        if self.macro_imports.has_external_glob || self.has_macro_use_extern {
            return false;
        }
...
impl<'ast> syn::visit::Visit<'ast> for Inclusions<'ast> {
    ...
    fn visit_item_macro(&mut self, node: &'ast syn::ItemMacro) {
        ...
        // `macro_rules` path itself, an external-looking name). Attributes
        // are shapes the invocation and shape scans cannot see — an
        // external procedural attribute expands invisibly at every
        // invocation site — so attribute-shaped tokens get the same
        // treatment (Codex P2, `review_comment` 3998151689).
        if node.mac.path.is_ident("macro_rules") {
            if tokens_could_include(&node.mac.tokens, &self.include_aliases)
                ...
                || !macro_body_attributes_are_accounted_for(self, &node.mac.tokens)
            {
                self.unresolvable = true;
            }
            // The early return below skips the `record` call that classifies
            // a macro's own attributes — but a procedural attribute on the
            // definition expands invisibly just like one in its body, so
            // each definition attribute gets the same `AttributeGate`
            // treatment instead (Codex P2, `review_comment` 3998151689).
...
fn include_declarations<'ast>(
    ...
    file_dir: &Path,
    local_macros: &HashSet<String>,
    macro_use_extern_in_root: bool,
) -> Option<Vec<(&'ast [syn::Attribute], PathBuf)>> {
    let shadowed_roots = shadowed_path_roots(file);
    let mut inclusions = Inclusions {
        file_dir: file_dir.to_path_buf(),
        found: Vec::new(),
        unresolvable: false,
        include_aliases: include_aliases(file, &shadowed_roots),
        local_macros: local_macros.clone(),
        macro_imports: external_macro_imports(file, &shadowed_roots),
        has_macro_use_extern: macro_use_extern_in_root || has_macro_use_extern_crate(file),
        shadowed_roots,
    };
...
/// `review_comment` 3997890782). The root is trusted to name the builtin
/// namespace only when no local item shadows it: `mod core { pub use
/// evil::include; }` plus `use core::include as paste;` makes `paste` name
/// evil's macro, so the alias is refused under a shadowed `core`/`std` root
/// (Codex P2, `review_comment` 3999003980).
fn include_aliases(file: &syn::File, shadowed_roots: &HashSet<String>) -> HashSet<String> {
    fn walk(
        tree: &syn::UseTree,
        root: Option<&String>,
        shadowed_roots: &HashSet<String>,
        ...
        match tree {
            syn::UseTree::Path(path) => {
                let next = root.cloned().or_else(|| Some(path.ident.to_string()));
                walk(&path.tree, next.as_ref(), shadowed_roots, aliases);
            }
            syn::UseTree::Name(_) | syn::UseTree::Glob(_) => {}
            syn::UseTree::Rename(rename) => {
                if rename.ident == "include"
                    && root.is_some_and(|name| {
                        (name == "core" || name == "std") && !shadowed_roots.contains(name)
                    })
                {
            ...
            syn::UseTree::Group(group) => {
                for item in &group.items {
                    walk(item, root, shadowed_roots, aliases);
                }
            }
        }
    }
    ...
    impl<'ast> syn::visit::Visit<'ast> for Collector<'_> {
        fn visit_item_use(&mut self, node: &'ast syn::ItemUse) {
            walk(&node.tree, None, self.shadowed_roots, &mut self.aliases);
            syn::visit::visit_item_use(self, node);
        }
    }
...
fn nested_invocation_paths(tokens: &proc_macro2::TokenStream) -> Vec<Option<syn::Path>> {
    ...
    fn is_pound(tree: &proc_macro2::TokenTree) -> bool {
        // The `#` of an inner attribute's `#!`.
        matches!(
            tree,
    ...
    fn walk(tokens: &proc_macro2::TokenStream, paths: &mut Vec<Option<syn::Path>>) {
        ...
        while let Some(tree) = trees.get(index) {
            // A `!` immediately after a `#` is an inner attribute's `#!`,
            // not a macro invocation: reading it as one yields a pathless
            // (`None`) invocation that fails the exemption closed — a false
            // positive on every builtin inner attribute a body carries.
            // [`macro_body_attributes`] pairs and classifies that shape
            // instead, failing closed on whatever it cannot parse, so the
            // invocation walk skips a `!` its `#` precedes. This opens no
            // hole: `#` is not a path token, so no invocation path can hide
            // behind it (Codex P2, `review_comment` 3998151689).
            let inner_attribute_bang = is_bang(tree)
                && index
                    .checked_sub(1)
                    .and_then(|prev| trees.get(prev))
                    .is_some_and(is_pound);
            if !inner_attribute_bang
                && is_bang(tree)
                && trees.get(index + 1).is_some_and(is_invocation_group)
            {
...
/// Every attribute written inside opaque tokens (a `macro_rules!` body).
///
/// An attribute is a `#` — or, for an inner attribute, `#!` — immediately
/// followed by a bracketed group. `None` marks a `#` the checker cannot
/// pair with a bracketed group, or one whose group does not parse as an
/// attribute: opaque tokens may hold anything, so an unparseable
/// attribute shape fails closed at classification time, like every other
/// shape the checker cannot enumerate. Groups are descended into, so
/// attributes nested inside an invocation's arguments or another group
/// are found too — but a paired group's own tokens are not re-scanned:
/// they are the attribute just classified.
fn macro_body_attributes(tokens: &proc_macro2::TokenStream) -> Vec<Option<syn::Attribute>> {
    fn is_pound(tree: &proc_macro2::TokenTree) -> bool {
        // Either spacing: an outer attribute's `#` lexes `Alone`, an inner
        // attribute's `#!` lexes `#` `Joint` with the `!`. Both are
        // attribute-shaped; anything else the `#` turns out to be fails
        // closed at classification time.
        matches!(
            tree,
            proc_macro2::TokenTree::Punct(punct) if punct.as_char() == '#'
    ...
    fn parse_attribute(
        pound: &proc_macro2::TokenTree,
        bang: Option<&proc_macro2::TokenTree>,
        group: &proc_macro2::Group,
    ) -> Option<syn::Attribute> {
        let mut stream = proc_macro2::TokenStream::new();
        stream.extend([pound.clone()]);
        if let Some(bang) = bang {
            stream.extend([bang.clone()]);
        }
        stream.extend([proc_macro2::TokenTree::Group(group.clone())]);
        let parse = if bang.is_some() {
            syn::Attribute::parse_inner
        } else {
            syn::Attribute::parse_outer
        };
        let mut parsed = parse.parse2(stream).ok()?;
        // `parse_inner` / `parse_outer` return every attribute in the
        // stream; the shape we paired holds exactly one.
        (parsed.len() == 1).then(|| parsed.remove(0))
    }

    fn walk(tokens: &proc_macro2::TokenStream, attrs: &mut Vec<Option<syn::Attribute>>) {
        let trees: Vec<proc_macro2::TokenTree> = tokens.clone().into_iter().collect();
        let mut index = 0;
        while let Some(tree) = trees.get(index) {
            if is_pound(tree) {
                let mut after = index + 1;
                let bang = if trees.get(after).is_some_and(is_bang) {
                    after += 1;
                    trees.get(after - 1)
                } else {
                    None
                };
                let parsed = match trees.get(after) {
                    Some(proc_macro2::TokenTree::Group(group))
                        if group.delimiter() == proc_macro2::Delimiter::Bracket =>
                    {
                        parse_attribute(tree, bang, group)
                    }
                    _ => None,
                };
                attrs.push(parsed);
                // Skip the paired tokens: a `#` inside the attribute's own
                // brackets is part of the attribute just classified.
                index = after + 1;
                continue;
            }
            if let proc_macro2::TokenTree::Group(group) = tree {
                walk(&group.stream(), attrs);
            }
            index += 1;
        }
    }
    ...
    walk(tokens, &mut attrs);
    attrs
}

/// Whether every attribute written inside opaque tokens (a `macro_rules!`
/// body) is one the checker can account for.
///
/// A procedural attribute's expansion is invisible the same way a
/// function-like macro's is: `macro_rules! wrapper { () => {
/// #[evil::startup] fn x() {} } }` applies the attribute at every
/// invocation site, and the expansion could declare `#[path =
/// "xtensa.rs"] mod arm_startup;` on ARM — while the invocation walk only
/// ever sees `wrapper!()`, trusted as a local macro, and the attribute
/// walk only visits parsed attributes, which opaque body tokens never are
/// (Codex P2, `review_comment` 3998151689). Each attribute-shaped token is
/// parsed and classified by [`AttributeGate`], the same rule the top-level
/// walk applies — builtins ignored, `derive` and `cfg_attr` accounted
/// for, anything else fail-closed — and anything the checker cannot parse
/// fails closed like every other shape it cannot enumerate.
fn macro_body_attributes_are_accounted_for(
    inclusions: &Inclusions<'_>,
    tokens: &proc_macro2::TokenStream,
) -> bool {
    let gate = AttributeGate {
        macro_imports: &inclusions.macro_imports,
        shadowed_roots: &inclusions.shadowed_roots,
    };
    macro_body_attributes(tokens).iter().all(|attr| {
        attr.as_ref()
            .is_some_and(|attr| gate.is_accounted_for(attr))
    })
}
...
mod tests {
    ...
    #[test]
    fn the_xtensa_bss_clear_uses_address_arithmetic() {
        // Codex P2 (review_comment 3998898841): `static mut _sbss: u32` declares
        // a lone `u32`, so a pointer derived from it carries provenance for that
        // single object only — `ptr::add` past it is undefined behavior from
        // the second word on, even though the linker lays `.bss` out
        // contiguously after it. The clear must therefore iterate over integer
        // addresses (rebuilding the write pointer from each exposed address)
        // rather than advancing a cursor with `ptr::add`.
        let startup_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../crates/waymaker-emu/src/xtensa.rs");
        let startup = std::fs::read_to_string(&startup_path).expect("xtensa.rs is committed");
        let fn_start = startup.find("fn zero_bss()").expect("zero_bss exists");
        let body_end = startup[fn_start..]
            .find("\nfn ")
            .map_or(startup.len(), |index| fn_start + index);
        let body = &startup[fn_start..body_end];
        assert!(
            body.contains("as usize"),
            "zero_bss must reduce the linker bounds to integer addresses before looping"
        );
        assert!(
            body.contains("as *mut u32"),
            "zero_bss must rebuild the write pointer from each address, not inherit provenance from _sbss"
        );
        assert!(
            body.contains("write_volatile"),
            "zero_bss must still clear with volatile word writes"
        );
        assert!(
            !body.contains(".add("),
            "zero_bss must not advance a pointer with ptr::add: provenance covers only the lone _sbss object"
        );
    }
    ...
    #[test]
    fn a_stale_log_is_removed_before_the_run() {
        // Codex P2 (review_comment 3998828462): a stale `uart0.log` carrying a
        // complete census would read as this run's census, so the run starts
        // with it gone.
        let dir =
            std::env::temp_dir().join(format!("waymaker-stale-log-removed-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let log = dir.join("uart0.log");
        std::fs::write(&log, xtensa_transcript()).unwrap();
        assert!(remove_stale_log(&log).is_ok());
        assert!(!log.exists(), "the stale log is gone before QEMU starts");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn a_missing_stale_log_is_not_an_error() {
        let dir =
            std::env::temp_dir().join(format!("waymaker-stale-log-missing-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let log = dir.join("uart0.log");
        assert!(!log.exists());
        assert!(remove_stale_log(&log).is_ok());
    ...
    #[test]
    fn an_unremovable_stale_log_fails_the_run_before_qemu_starts() {
        // Codex P2 (review_comment 3998828462): ignoring the removal error lets
        // the first polling iteration parse the old census before QEMU has
        // opened and truncated the file, so the harness kills the new guest
        // and reports a successful run that never produced the observed
        // output. Only a missing log is fine.
        let dir =
            std::env::temp_dir().join(format!("waymaker-stale-log-blocked-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        // A regular file where the parent directory should be: `remove_file`
        // fails with `NotADirectory` deterministically, without depending on
        // running as a user whose permissions the kernel enforces.
        let blocker = dir.join("blocker");
        std::fs::write(&blocker, b"not a directory").unwrap();
        let log = blocker.join("uart0.log");
        let error = remove_stale_log(&log).expect_err("an unremovable stale log must fail the run");
        assert!(error.contains("uart0.log"), "{error}");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn a_macro_use_extern_crate_in_the_root_shadows_builtins_in_child_files() {
        // Codex P2 (review_comment 3998577504): `#[macro_use] extern crate`
        // in the crate root puts the dependency's macros in textual scope
        // for every module, so a bare `assert!()` in `arm.rs` may invoke the
        // dependency's macro — not the builtin — even though `arm.rs` itself
        // carries no `#[macro_use]`. The builtin whitelist must not apply
        // there: the invocation fails closed like any other unresolvable one.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[macro_use]\n\
        ...
        let sources = vec![
            ...
            LayerSource {
                crate_name: PACKAGE.to_owned(),
                path: format!("crates/{PACKAGE}/src/arm.rs"),
                contents: "fn check() { assert!(true); }\n".to_owned(),
            },
            LayerSource {
                crate_name: PACKAGE.to_owned(),
                path: format!("crates/{PACKAGE}/src/xtensa.rs"),
        ...
        assert_eq!(
            violations.len(),
            1,
            "a builtin name shadowed by #[macro_use] extern crate in the root is not the builtin in a child file: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_cfg_attr_macro_use_extern_crate_in_the_root_shadows_builtins_in_child_files() {
        // Codex P2 (review_comment 3999109488): `#[cfg_attr(target_arch =
        // "arm", macro_use)] extern crate evil;` emits `#[macro_use]` when
        // the predicate holds, and rustc puts the dependency's macros in
        // textual scope for every module exactly like a directly written
        // `#[macro_use]`. The checker does not evaluate predicates, so the
        // `cfg_attr` form must count the same way: a bare `assert!()` in
        // `arm.rs` may invoke the dependency's macro, not the builtin.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
        ...
        assert_eq!(
            violations.len(),
            1,
            "a builtin name shadowed by cfg_attr-emitted #[macro_use] extern crate in the root is not the builtin in a child file: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_nested_cfg_attr_macro_use_extern_crate_counts_as_macro_use() {
        // Codex P2 (review_comment 3999109488), nested form: rustc expands a
        // `cfg_attr` inside the emitted list in place, so `#[cfg_attr(arm,
        // cfg_attr(unix, macro_use))]` can still emit `#[macro_use]` and
        // must count the same way.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg_attr(target_arch = \"arm\", cfg_attr(unix, macro_use))]\n\
             extern crate evil;\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod arm;\n"
        ...
        assert_eq!(
            violations.len(),
            1,
            "a builtin name shadowed by nested cfg_attr-emitted #[macro_use] extern crate in the root is not the builtin in a child file: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_cfg_attr_extern_crate_without_macro_use_keeps_the_gate_green() {
        // Codex P2 (review_comment 3999109488), negative guard: a `cfg_attr`
        // that emits anything other than `macro_use` must not trip the
        // wholesale-import handling — `assert!()` stays the builtin and the
        // gate stays green.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "a cfg_attr extern crate that does not emit macro_use keeps the gate green: {violations:?}",
        );
    }

    #[test]
    fn a_macro_generated_macro_use_extern_crate_shadows_an_unrelated_local_macro() {
        // Codex P2 (review_comment 3999217503): a `macro_rules!` body is
        // opaque tokens to `syn`, so `has_macro_use_extern_crate` cannot see
        // a `#[macro_use] extern crate evil;` the macro generates at its
        // invocation site. With an unrelated same-named local `macro_rules!
        // load_startup` elsewhere in the crate, a bare `load_startup!()` in
        // `arm.rs` resolves to the dependency's textual-scope macro — not
        // the local definition — so the crate-wide local-name set must not
        // trust it: the invocation fails closed like any other unresolvable
        // one.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             macro_rules! bring_evil {{ () => {{ #[macro_use] extern crate evil; }}; }}\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod arm;\n\
             mod helper;\n"
        ...
        let sources = vec![
            ...
            LayerSource {
                crate_name: PACKAGE.to_owned(),
                path: format!("crates/{PACKAGE}/src/arm.rs"),
                contents: "fn check() { load_startup!(); }\n".to_owned(),
            },
            LayerSource {
                crate_name: PACKAGE.to_owned(),
                path: format!("crates/{PACKAGE}/src/helper.rs"),
        ...
        assert_eq!(
            violations.len(),
            1,
            "a macro-generated #[macro_use] extern crate shadows an unrelated same-named local macro: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_macro_body_naming_the_tokens_apart_keeps_the_gate_green() {
        // Codex P2 (review_comment 3999217503), negative guard: the
        // macro-body scan matches the `#[macro_use] extern crate` token
        // shape, not the tokens apart. A body that binds `macro_use` as a
        // plain identifier and declares a plain `extern crate` generates no
        // wholesale import, so the bare `load_startup!()` still resolves to
        // the unrelated local macro and the gate stays green.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             macro_rules! notes {{ () => {{ let macro_use = 1; extern crate evil; let _ = macro_use; }}; }}\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod arm;\n\
             mod helper;\n"
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "a macro body naming the tokens apart is not a generated #[macro_use] extern crate: {violations:?}",
        );
    }

    #[test]
    fn a_local_module_shadowing_a_builtin_namespace_root_refuses_the_exemption() {
        // Codex P2 (review_comment 3998765648): a local item shadows the
        // extern prelude for path resolution, so `core::assert!()` with
        // `mod core { pub use evil::assert; }` in the file invokes the
        // re-exported foreign macro — not the compiler builtin. The builtin
        // whitelist must not apply there: the invocation fails closed like
        ...
        let sources = vec![
            ...
            LayerSource {
                ...
                path: format!("crates/{PACKAGE}/src/arm.rs"),
                contents:
                    "mod core { pub use evil::assert; }\nfn check() { core::assert!(true); }\n"
                        .to_owned(),
            },
            LayerSource {
                crate_name: PACKAGE.to_owned(),
        ...
        assert_eq!(
            violations.len(),
            1,
            "a builtin namespace root shadowed by a local module is not the builtin: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_wholesale_macro_import_beats_the_crate_wide_local_name_set() {
        // Codex P2 (review_comment 3998765650): with `#[macro_use]
        // extern crate evil;` in the root, a bare invocation in a child
        // module resolves to the dependency's textual-scope macro even when
        // an unrelated module defines a same-named `macro_rules!` — the
        // crate-wide over-approximated local-name set does not put the local
        // definition in textual scope at the invocation site. The wholesale
        // import must be checked before the local-name set: the invocation
        // fails closed like any other unresolvable one.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[macro_use]\n\
        ...
        assert_eq!(
            violations.len(),
            1,
            "a wholesale macro import shadows an unrelated same-named local macro: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn an_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption() {
        // Codex P2 (review_comment 3998151689): a `macro_rules!` body is
        // opaque tokens, and the definition scan looked for `include!` /
        // `mod` shapes and macro-invocation shapes in it — but an attribute
        // is none of those: `macro_rules! wrapper { () => { #[evil::startup]
        // fn x() {} } }` smuggled an external procedural attribute past
        // every scan, while the attribute walk only visits parsed
        // attributes, which opaque body tokens never are — and the later
        // `wrapper!()` is trusted as a local macro, while the expansion's
        // invisible expansion could declare `#[path = "xtensa.rs"]
        // mod arm_startup;` on ARM. Attribute-shaped tokens in the body are
        // classified by the same `AttributeGate` the top-level walk applies;
        // anything unaccounted for fails the exemption closed.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #[evil::startup] fn x() {{}} }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations.len(),
            1,
            "an external attribute nested in a macro_rules! body fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_builtin_attribute_nested_in_a_macro_rules_body_keeps_the_exemption() {
        // A builtin attribute is evaluated by rustc, not by an external
        // expansion, so it can neither declare the gated module nor paste
        // it — the definition scan still accounts for the local macro
        // around it.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #[allow(unused)] fn x() {{}} }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "a builtin attribute nested in a macro_rules! body stays accounted for: {violations:?}",
        );
    }

    #[test]
    fn a_std_derive_nested_in_a_macro_rules_body_keeps_the_exemption() {
        // A std derive expands to trait impls only — no items that could
        // declare the gated module — so a `#[derive(...)]` written inside a
        // `macro_rules!` body is classified by the same derive rule the
        // top-level attribute walk applies.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #[derive(Clone, Copy)] struct S(u8); }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "a std derive nested in a macro_rules! body stays accounted for: {violations:?}",
        );
    }

    #[test]
    fn a_cfg_attr_emitting_an_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption()
     {
        // `cfg_attr` is evaluated by rustc: the attributes it emits are
        // classified like any attribute written out by hand — so a
        // `cfg_attr` inside a `macro_rules!` body that can emit an external
        // procedural attribute fails the exemption closed the same way a
        // handwritten hostile attribute does.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #[cfg_attr(target_arch = \"arm\", evil::startup)] fn x() {{}} }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations.len(),
            1,
            "a cfg_attr emitting an external attribute nested in a macro_rules! body fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn an_inner_external_attribute_nested_in_a_macro_rules_body_refuses_the_exemption() {
        // An inner attribute lexes its `#` joint with the `!`, not alone —
        // the scanner pairs either spacing, or `#![evil::startup]` smuggles
        // the same external procedural attribute past the body scan that the
        // outer-attribute case covers.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #![evil::startup] }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations.len(),
            1,
            "an inner external attribute nested in a macro_rules! body fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn an_inner_builtin_attribute_nested_in_a_macro_rules_body_keeps_the_exemption() {
        // A builtin inner attribute is evaluated by rustc, not by an
        // external expansion, so it can neither declare the gated module nor
        // paste it — the definition scan still accounts for the local macro
        // around it.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             macro_rules! wrapper {{ () => {{ #![allow(unused)] }} }}\n\
             wrapper!();\n"
        );
        let sources = vec![
            LayerSource {
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "an inner builtin attribute nested in a macro_rules! body stays accounted for: {violations:?}",
        );
    }

    #[test]
    fn an_external_attribute_on_a_macro_rules_definition_refuses_the_exemption() {
        // Codex P2 (review_comment 3998151689), definition-side analogue: a
        // procedural attribute on the `macro_rules!` definition itself
        // expands invisibly just like one in its body, and
        // `visit_item_macro` returns early for `macro_rules!` without
        // visiting `node.attrs` — so `#[evil::startup] macro_rules!
        // wrapper { ... }` smuggled the external attribute past every scan
        // while the later `wrapper!()` is trusted as a local macro. Each
        // definition attribute is classified by the same `AttributeGate`
        // the top-level walk applies; anything unaccounted for fails the
        // exemption closed.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
        ...
        assert_eq!(
            violations.len(),
            1,
            "an external attribute on a macro_rules! definition fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_builtin_attribute_on_a_macro_rules_definition_keeps_the_exemption() {
        // A builtin attribute on the definition is evaluated by rustc, not
        // by an external expansion, so it can neither declare the gated
        // module nor paste it — classifying definition attributes must not
        // newly refuse the exemption for those.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
        ...
        assert_eq!(
            violations,
            Vec::new(),
            "a builtin attribute on a macro_rules! definition stays accounted for: {violations:?}",
        );
    }
    ...
    #[test]
    fn a_bare_attribute_imported_through_a_shadowed_crate_root_fails_closed() {
        // Codex P2 (review_comment 3999003978): `external_macro_imports`
        // recorded `use cortex_m_rt::entry;` as the trusted
        // `(cortex_m_rt, entry)` pair from the textual root alone, and the
        // bare attribute arm of `AttributeGate::is_accounted_for` granted the
        // pinned exemption on that pair — bypassing the `shadowed_roots`
        // check that guards qualified paths. `mod cortex_m_rt { pub use
        // evil::entry; }` shadows the extern prelude, so the bare `#[entry]`
        // applies evil's procedural macro, an unexamined expansion. An
        // import whose root is shadowed must classify as unresolved, not
        // pinned.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
        ...
        assert_eq!(
            violations.len(),
            1,
            "a pinned attribute imported through a shadowed crate root fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn a_bare_macro_imported_through_a_shadowed_crate_root_fails_closed() {
        // Codex P2 (review_comment 3999003978), macro side: the same
        // textual-root hole in `external_macro_imports` granted the pinned
        // external-macro exemption to a bare invocation. `mod
        // cortex_m_semihosting { pub use evil::hprintln; }` plus `use
        // cortex_m_semihosting::hprintln;` makes a bare `hprintln!()` apply
        // evil's macro, whose expansion the checker cannot read — so the
        // import must not resolve to the pinned `(crate, name)` pair.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
        ...
        assert_eq!(
            violations.len(),
            1,
            "a pinned macro imported through a shadowed crate root fails closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn conflicting_macro_imports_across_scopes_fail_closed() {
        // Codex P2 (review_comment 3999109484): the import walk is
        // file-wide, but separate lexical scopes may import the same local
        // name from different crates — which compiles fine. The last
        // `insert` must not silently win: the `hprintln!()` in the
        // `unvetted` module resolves to evil's macro, whose expansion the
        // checker cannot read, yet the later module's pinned
        // `(cortex_m_semihosting, hprintln)` mapping would classify it as
        // the vetted exemption. Disagreement fails closed.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod unvetted {{ use evil::hprintln; pub fn check() {{ hprintln!(); }} }}\n\
             mod vetted {{ use cortex_m_semihosting::hprintln; pub fn check() {{ hprintln!(); }} }}\n"
        );
        let sources = vec![
            LayerSource {
                crate_name: PACKAGE.to_owned(),
        ...
        assert_eq!(
            violations.len(),
            1,
            "conflicting macro imports for one local name fail closed: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn agreeing_macro_imports_across_scopes_keep_their_mapping() {
        // Codex P2 (review_comment 3999109484), negative guard: the same
        // (crate, name) mapping recorded from two scopes is not a conflict
        // — an unvetted crate stays unvetted, and the gate still fails
        // closed on it exactly once.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod first {{ use evil::hprintln; pub fn check() {{ hprintln!(); }} }}\n\
             mod second {{ use evil::hprintln; pub fn check() {{ hprintln!(); }} }}\n"
        );
        let sources = vec![
            LayerSource {
                crate_name: PACKAGE.to_owned(),
        ...
        assert_eq!(
            violations.len(),
            1,
            "agreeing macro imports keep their mapping: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"
    ...
    #[test]
    fn an_include_alias_through_a_shadowed_core_root_is_not_an_alias() {
        // Codex P2 (review_comment 3999003980): `include_aliases` recorded
        // `use core::include as paste;` as a rename of the builtin whenever
        // the root was spelled `core`, without consulting root shadowing.
        // `mod core { pub use evil::include; }` makes `paste` name evil's
        // macro, so `paste!("other.rs")` must fall through to the
        // external-macro classification — which fails closed — instead of
        // being recorded as a harmless literal inclusion.
        let root = format!(
            "#![no_std]\n#![no_main]\n#![allow(unsafe_code, reason = \"the reset vector\")]\n\
             pub const PREFIX: &str = \"{PREFIX}\";\n\
             #[cfg(target_arch = \"xtensa\")]\n\
             mod xtensa;\n\
             mod core {{ pub use evil::include; }}\n\
             use core::include as paste;\n\
             paste!(\"other.rs\");\n"
        );
        let sources = vec![
        ...
        assert_eq!(
            violations.len(),
            1,
            "an include alias through a shadowed core root is an unaccounted external macro: {violations:?}"
        );
        assert!(
            violations[0].subject.ends_with("src/xtensa.rs"),
            "{violations:?}"

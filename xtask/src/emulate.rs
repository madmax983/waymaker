    ...
    pub fn render(&self) -> String {
        ...
        out.push_str(
            "\nWhat this establishes is that the rig executes on both instruction sets, and that both\n\
             ...
             into different instructions. See ADR 0043.\n",
        );
        out
    }
}
...
/// name instead of being invisible to this scan the way a macro's own `unsafe` is. ADR 0043
...
/// this image actually owns, whatever the reading says — [ADR 0043]'s soundness fix for
...
mod tests {
    ...
    fn the_real_stack_module_passes() {
        ...
        // ADR 0043's own claim: the linker-symbol block, and `unsafe` confined to the two

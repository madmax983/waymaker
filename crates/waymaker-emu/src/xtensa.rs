...
///
/// The loop runs over integer addresses rather than advancing a pointer from
/// `_sbss`: each linker symbol is declared as a lone `u32`, so a pointer
/// derived from `_sbss` carries provenance for that single object only, and
/// `ptr::add` past it is undefined behavior from the second word on — even
/// though the linker lays `.bss` out contiguously after it (Codex P2,
/// `review_comment` 3998898841). Reducing the bounds to `usize` exposes the
/// addresses and casting each one back rebuilds the write pointer from the
/// exposed address instead of inheriting the wrong provenance. Both ends are
/// word-aligned by the linker script (`memory-xtensa.x`), so every volatile
/// word write stays inside the section.
fn zero_bss() {
    unsafe {
        let start = core::ptr::addr_of_mut!(_sbss) as usize;
        let end = core::ptr::addr_of_mut!(_ebss) as usize;
        let mut addr = start;
        while addr < end {
